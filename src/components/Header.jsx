...
const Header = () => {
    ...
    const scrollToSection = (e, sectionId) => {
        ...
        if (section) {
            window.scrollTo({
                top: section.offsetTop - 80, // Ajuste de 80px para compensar a altura do header
                behavior: 'smooth'
            });
            
            // Fecha o sidebar no mobile, se estiver aberto
            if (isSidebarOpen) {
                closeSidebar();
            }
        }
        ...
        <>
            <header 
                className={`bg-gray-700 h-18 p-3 flex justify-between md:justify-between items-center fixed top-0 left-0 right-0 z-10 transition-all duration-300 ${
                    isScrolled ? "bg-opacity-80 shadow-md" : "bg-opacity-80"
                }`}
            >   
            ...
            {/* Mobile sidebar */}
            <div 
                className={`fixed bg-gray-800 top-0 right-0 h-full w-64 shadow-lg transform transition-transform duration-300 ease-in-out z-30 ${
